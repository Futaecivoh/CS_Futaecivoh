...
### DRY
Чтобы не прописывать сотню раз `if` `else`, мы просто сделаем отдельную функцию. Это будет в разы удобнее!
...
***Все это поможет нам повысить читаемость кода, ускорить его написание и сделать его более безопасным***

### Проверка
По ТЗ, мы должны написать небольшой скрипт, который автоматически проверяет уязвимость на сервере и смотрит, чтобы полные права имел только админ
и только он бы мог знать о наличие файлов
